  ...
  return (
    <div className="h-screen flex flex-col bg-[var(--bg-primary)] text-[var(--text-primary)]">
      ...
      {authLoading ? (
        ...
        <div className="flex-1 flex flex-col justify-center px-6 py-8">
          <div className="text-center mb-6">
            ...
            <p className="text-xs text-[var(--text-tertiary)]">Sign in to your LeadScaper account</p>
          </div>
          <form onSubmit={handleLogin} className="space-y-3">
            {authError && <div className="text-xs text-red-500 bg-red-500/10 p-2 rounded">{authError}</div>}
            <Input
              placeholder="Email"
              type="email"
              value={email}
            ...
            <Input
              ...
              icon={<Lock size={13} />}
            />
            <Button variant="primary" className="w-full" type="submit" disabled={!email || !password || authLoading}>
      ...
      <div className="px-4 py-2 border-t border-[var(--border-secondary)] flex gap-2 flex-shrink-0">
        ...
        <Button variant="destructive" size="sm" onClick={handleLogout} className={leads.length === 0 ? "flex-1" : ""}>
          Logout
        </Button>
      </div>
        </>
